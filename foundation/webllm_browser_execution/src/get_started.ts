...
async function loadEngine() {
  ...
  const appConfig: webllm.AppConfig = {
    model_list: [
      ...
      {
        model_id: "Phi-3.5-git",
        model: window.location.origin + "/models/MLC/Phi-3.5-git-MLC",
        model_lib: window.location.origin + "/models/libs/Phi-3.5-git-webgpu.wasm",
      },
    ],
  };
  const mlc_engine: webllm.MLCEngineInterface = await webllm.CreateMLCEngine("Llama-3.2-local-lib", { appConfig: appConfig, initProgressCallback: initProgressCallback },);
