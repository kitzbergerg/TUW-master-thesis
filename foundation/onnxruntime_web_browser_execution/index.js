import { GPT2Tokenizer } from '@huggingface/transformers';
import * as ort from 'onnxruntime-web';



async function createSession(modelPath, preferredOutputLocation, executionProviders = "webnn") {
    return await ort.InferenceSession.create(modelPath, {
        executionProviders: [executionProviders],
        preferredOutputLocation: preferredOutputLocation
    });
}

async function runModel(session, feeds) {
    return await session.run(feeds);
}

function argMax(array) {
    return [].reduce.call(array, (m, c, i, arr) => c > arr[m] ? i : m, 0)
}
async function runGpt(text) {
    // TODO: figure out how to use webnn (not working)
    // TODO: figure out how to use webgpu (really slow)
    // TODO: use kv cache?
    const session = await createSession('model/gpt2.onnx', 'cpu', 'wasm');

    const tokenizer = await GPT2Tokenizer.from_pretrained('gpt2');
    let encoded = await tokenizer(text).input_ids;
    for (let i = 0; i < 20; i++) {
        const outputs = await runModel(session, { input_ids: encoded })

        const logits = outputs.logits.cpuData;
        const vocabSize = outputs.logits.dims[2];
        const logits_next_token = logits.slice(-vocabSize)
        const nextToken = argMax(logits_next_token);
        console.log("Generated token IDs:", nextToken);
        console.log(tokenizer.decode([nextToken]))

        const data = BigInt64Array.from([...encoded.data, BigInt(nextToken)])
        encoded = new ort.Tensor('int64', data, [1, data.length]);
    }
    return tokenizer.decode([...encoded.cpuData])
}
async function startGpt() {
    const prompt = document.getElementById("prompt").value;
    const text = await runGpt(prompt);

    document.getElementById("pre").innerHTML = text;
}
window.startGpt = startGpt



function arrayEquals(a, b) {
    return a.length === b.length && a.every((val, index) => val === b[index]);
}

async function testModels() {
    try {
        await testSqueezenet();
        await testGpt2();
    } catch (e) {
        console.error(e)
    }
}
window.testModels = testModels

async function testSqueezenet() {
    const dims = [1, 3, 224, 224];
    const data = Float32Array.from(Array(dims.reduce((acc, el) => acc * el)).fill(0));
    const tensor = new ort.Tensor('float32', data, dims);
    const feeds = { data: tensor };

    // Test full squeezenet
    const session = await createSession('model/squeezenet1.1-7.onnx', 'cpu', 'webnn');
    const out = await runModel(session, feeds);
    console.log(out);

    // Test split squeezenet
    const session_p1 = await createSession('model/squeezenet1.1-7_p1.onnx', 'ml-tensor', 'webnn');
    const out_p1 = await runModel(session_p1, feeds);
    console.log(out_p1);

    const session_p2 = await createSession('model/squeezenet1.1-7_p2.onnx', 'cpu', 'webnn');
    const out_p2 = await runModel(session_p2, out_p1);
    console.log(out_p2);

    // assert
    console.assert(arrayEquals(out.squeezenet0_flatten0_reshape0.cpuData, out_p2.squeezenet0_flatten0_reshape0.cpuData));
    console.log("Split squeezenet gives same result as full variant")
}

async function testGpt2() {
    const text = "Why is the sky blue?";
    const tokenizer = await GPT2Tokenizer.from_pretrained('gpt2');
    const encoded = await tokenizer(text).input_ids;
    const feeds = { input_ids: encoded };

    // Test full gpt2
    const session = await createSession('model/gpt2.onnx', 'cpu', 'wasm');
    const out = await runModel(session, feeds);
    console.log(out);


    // Test split gpt2
    const session_p1 = await createSession('model/gpt2_p1.onnx', 'cpu', 'wasm');
    const out_p1 = await runModel(session_p1, feeds);
    console.log(out_p1);

    const session_p2 = await createSession('model/gpt2_p2.onnx', 'cpu', 'wasm');
    const out_p2 = await runModel(session_p2, out_p1);
    console.log(out_p2);

    // assert
    console.assert(arrayEquals(out.logits.cpuData, out_p2.logits.cpuData));
    console.log("Split gpt2 gives same result as full variant")
}
